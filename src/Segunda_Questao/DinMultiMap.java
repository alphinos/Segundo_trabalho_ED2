...
public class DinMultiMap<Key, Value> {
    ...
    public DinMultiMap( int capacity, int increment ){
    ...
    public void put(Key key, Value value ){
        ...
        if ( this.registers.get(index) == null ){                       this.comparisons++;
            this.registers.set(index, new List<>());                    this.assignments++;
            this.totalIndex++;
        } else {
            if ( data.compareTo( this.registers.get( index ).getFirst() ) != 0 ){
                for(int k=1;k<this.max_size;k++){
                    int indexQuadratico=hashQuadratico(key,k);
                    if(this.registers.get(indexQuadratico)==null){
                        this.registers.set(index, new List<>());                    this.assignments++;
                        this.totalIndex++;
                        break;
                    }
